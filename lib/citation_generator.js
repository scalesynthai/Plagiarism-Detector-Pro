...
class CitationGenerator {
    ...
    static async resolveCitation(query) {
        ...
        let journal = "Unknown publication";
        ...
        try {
            if (doi) {
                ...
                const res = await fetch(url, { signal: AbortSignal.timeout(5000), headers: { "User-Agent": "PlagiarismDetectorPro/1.0 (mailto:support@subba.dev)" } });
                if (res.ok) {
                    const data = await res.json();
                    const item = data.message || {};
                    metadataResolved = Boolean(item.title && item.title[0]);
  ...
  year      = {${year}}${doi ? `,\n  doi       = {${doi}}` : ""}
        ...
        return {
            ...
            warning: metadataResolved ? null : "Metadata lookup unavailable. This is an unverified template; verify every field before citing.",
            title,
            authors,
            journal,
            year,
