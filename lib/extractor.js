...
class DocumentExtractor {
    ...
    static extractFromFile(filePath) {
        ...
        if (![".txt", ".md", ".tex", ".ipynb", ".json", ".csv", ".bib"].includes(ext)) {
            throw new Error(`Unsupported local document format: ${ext}. Use the web application for PDF and DOCX.`);
        }
        if (!fs.statSync(filePath).isFile() || fs.statSync(filePath).size > 8 * 1024 * 1024) {
