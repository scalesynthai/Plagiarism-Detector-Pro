...
class AcademicStudentCoach {
    ...
    static scanUnsupportedClaims(text) {
        if (!text) return [];
        const sentences = require('./matching').sentenceRanges(text).map(([a,b])=>text.slice(a,b).trim()).filter(s=>s.length>20);

        const unsupported = [];

        for (const s of sentences) {
            const hasCite = require('./diagnostics').citations(s).length > 0;
            if (hasCite) continue;
    ...
    static evaluateThesisAbstract(text) {
        ...
        const opening = require('./diagnostics').selectOpening(text);
        const firstPara = opening.text;
        const words = firstPara.trim().split(/\s+/).filter(Boolean);
        const wordCount = words.length;

        const hasHypothesis = /\b(hypothesi[sz](?:e|es)|argu(?:e|es)|propos(?:e|es)|demonstrat(?:e|es)|investigat(?:e|es)|examin(?:e|es)|aims? to|objective|central claim|research question)\b/i.test(firstPara);
        ...
        return {
            ...
            evaluated_text: firstPara, selection: opening.selection, start: opening.start,
