...
class BatchProcessor {
     ...
     */
    static processBatch(targetPaths, options = {}) {
        let fileList = [];
        ...
        fileList.forEach(filePath => {
            try {
                ...
                const plagRes = checker.analyze(text, options);
        ...
        const successCount = gradebook.filter(row => !row.error).length;

        return {
            total_submissions: totalSubs,
            processed_successfully: successCount,
            average_plagiarism: parseFloat((totalPlag / Math.max(1, successCount)).toFixed(1)),
            average_ai_probability: parseFloat((totalAi / Math.max(1, successCount)).toFixed(1)),
